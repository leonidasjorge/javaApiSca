...
public class DisciplinaConverter implements CrudConverter<Disciplina, DisciplinaDTO> {
	...
	public DisciplinaDTO entidadeParaDTO(Disciplina entidade) {		
		return new DisciplinaDTO(entidade.getId(), entidade.getCargaHoraria(), entidade.getDataCadastro(), entidade.getNome());
	}

	@Override
	public Disciplina dtoParaEntidade(DisciplinaDTO dto) {
		return new Disciplina(dto.getId(), dto.getCargaHoraria(), dto.getDataCadastro(), dto.getNome());
	}
